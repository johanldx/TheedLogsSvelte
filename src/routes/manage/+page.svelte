<script lang="ts">
                    ...
                    columns: [
                        ...
                        'Organisation',
                        {
                            ...
                            formatter: (_, row) => html(`
                                <button class="bg-primary text-white px-2 py-1 rounded" onclick="openEditModal(${row.cells[1].data}, '${row.cells[0].data}')">Modifier</button>
                                <button class="bg-red-500 text-white px-2 py-1 rounded" onclick="openDeleteModal(${row.cells[1].data})">Supprimer</button>
                            `),
                        },
                    ],
    ...
    function openDeleteModal(appId: number) {
        selectedAppId = appId;
...
<div class="modal fixed top-0 left-0 w-full h-full flex items-center justify-center bg-black bg-opacity-50" class:visible={isDeleteModalOpen}>
    <div class="modal-content bg-white rounded-lg shadow-lg p-8 max-w-md w-full">
        <h2 class="text-lg font-semibold mb-4">Supprimer l'application</h2>
        <p class="text-gray-700 mb-6">Êtes-vous sûr de vouloir supprimer cette application ? Cette action est irréversible.</p>
        <div class="flex justify-center space-x-3">
